const es = {
  'firstPage': {
    'eslogan': 'Respira. Registra. Conecta.',
    'text': 'En un mundo que nunca se detiene, Moodify es tu refugio digital. Un espacio diseñado en tonos suaves para que encuentres la claridad que necesitas, un día a la vez.',
    'button': 'Comenzar mi viaje'
  },
  'panel':{
    'hello': '¡Hola!',
    'welcome': 'Bienvenid@',
    'unite': 'Únete',
    'enter': 'Entra'
  },
  'message':{
    'fieldRequiredMessageFull': '¡A un paso! Parece que falta algún campo por rellenar para poder continuar.',
    'fieldRequired': '¡Ups! Parece que olvidaste completar este campo.',
    'fieldError': '¡Casi lo tienes! Revisa los detalles marcados en rojo para continuar.',
    'correctRegister': '¡Ya casi eres parte de Moodify! 🌈 Solo un paso más: verifica tu cuenta desde el correo que te acabamos de enviar.',
    'correctLogin': 'Bienvenido a casa. Todo está justo donde lo dejaste.', 
    'incorrectCredentials':'Credenciales incorrectas',
    'notVerified':'Todavia no has verificado la cuenta',
    'register':{
      'emailFormatInvalid': 'El formato del correo electrónico no es válido.',
      'accountAlreadyExistEmail': 'Ya existe una cuenta con este correo. Prueba con otro',
      'accountAlreadyExistUsername': 'Este nombre de usuario ya está en uso. Prueba con otro.',
      'passwordDontWork': 'La contraseña debe tener al menos 8 caracteres, una letra y un símbolo (ej: !@#$).'
    },
    
  },
  'loginRegister':{
      'user': 'Usuario',
      'completeName': 'Nombre completo',
      'email': 'Correo electrónico',
      'password': 'Contraseña',
      'repeatPassword': 'Repite la contraseña',
      'passwordsDontMatch': 'Las contraseñas no coinciden',
      'join': 'Únete',
      'weAlreadyKnowEachOther': '¿Ya nos conocemos?',
      'login': 'Inicia sesión',
      'userOrEmail': 'Usuario o email',
      'dontHaveAccount': '¿No tienes cuenta?',
  },
  'calendarGrid':{
    howAreYou: '¿Cómo estás?',
    youFeel: 'Te sientes',
    todaysNote: 'Nota de hoy...',
    saveStatus: 'Guardar estado',
    summaryMonth: 'Resumen del mes',
    viewHistory: 'Ver historial',
    todaysactivity: 'Actividad de hoy'

  },
  'moodNames': {
  'happy': 'Feliz',
  'glad': 'Bien',
  'excited': 'Emocionad@',
  'calm': 'Tranquil@',
  'proud': 'Orgullos@',
  'confident': 'Segur@',
  'grateful': 'Agradecid@',
  'in_love': 'Enamorad@',
  'peaceful': 'En paz',
  'sad': 'Triste',
  'angry': 'Enfadad@',
  'scared': 'Asustad@',
  'anxious': 'Ansios@',
  'annoyed': 'Molest@',
  'frustrated': 'Frustrad@',
  'embarrassed': 'Avergonzad@',
  'lonely': 'Sol@',
  'worried': 'Preocupad@',
  'furious': 'Furios@',
  'tired': 'Cansad@',
  'sleepy': 'Somnolient@',
  'bored': 'Aburrid@',
  'confused': 'Confundid@',
  'surprised': 'Sorprendid@',
  'serious': 'Seri@',
  'shy': 'Tímid@',
  'hungry': 'Hambrient@'
}
}
export default es;